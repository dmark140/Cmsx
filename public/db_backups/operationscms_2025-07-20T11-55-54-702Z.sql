...
select
  ...
  `a`.`DocEntry` AS `DocEntry`,case
  when count(if(`a`.`decision` = 2, 1, NULL)) > 0 then 'Rejected'
  when `b`.`type` = 2
  and count(if(`a`.`decision` = 1, 1, NULL)) > 0 then 'Approved'
  when `b`.`type` = 1
  and count(if(`a`.`decision` = 1, 1, NULL)) = count(`a`.`ApprovalApproverID`) then 'Approved'
  else 'Pending'
  end AS `FinalApprovalStatus`
from
  (
  (
...
select
  `sub`.`DocEntry` AS `DocEntry`,
  `sub`.`projectTitle` AS `projectTitle`,
  `sub`.`Disc` AS `Disc`,
  `sub`.`Requirements_id` AS `Requirements_id`,
  `sub`.`Requirements_value` AS `Requirements_value`,
  `sub`.`title` AS `title`,
  `sub`.`type` AS `type`,
  `sub`.`name` AS `name`,
  `sub`.`iValue` AS `iValue`,
  `sub`.`dataNumber` AS `dataNumber`,
  `sub`.`rn` AS `rn`
from
  (
  select
...
select
  if(`b`.`Code` is null, `a`.`label`, `b`.`Code`) AS `label`,
  `a`.`DocEntry` AS `DocEntry`,
  `a`.`DocNum` AS `DocNum`
from
  (
...
VALUES
  (3, 3, 4, '2025-07-19 01:01:16', 0, 2, 1);
INSERT INTO
  `approvals_decisions` (
    `DocNum`,
    `ApprovalApproverID`,
...
VALUES
  (1, 'd@g.c', 'd', 'f', 'b', 'test', 1, 'dmark34907', 1);
INSERT INTO
  `ousr` (
    `DocEntry`,
    `email`,
...
# ------------------------------------------------------------
# TRIGGER DUMP FOR: approval_decision_trigger_userVoid
# ------------------------------------------------------------

DROP TRIGGER IF EXISTS approval_decision_trigger_userVoid;
DELIMITER ;;
CREATE TRIGGER `approval_decision_trigger_userVoid`
AFTER UPDATE ON `approvals_decisions`
FOR EACH ROW
BEGIN
  UPDATE ousr B
  JOIN vwdmf_approval_status A ON A.DocEntry = B.DocEntry
  SET B.void = 1
  WHERE 
    A.project_id = 1 
    AND A.FinalApprovalStatus = 'Approved' 
    AND B.void = 0;
END;;
DELIMITER ;
